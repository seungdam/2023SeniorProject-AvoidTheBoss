...
void ClientIocpCore::DispatchPackets(atb::ClientPacketDispatcher& dispatcher)
{
	if (_client)
	{
		_client->DispatchPackets(dispatcher);
	}
}
...
bool ClientIocpCore::PrepareSocket(const bool reconnect)
{
	...
	if (reconnect)
	{
		_client->ResetForReconnect(socket);
	}
	else
	{
...
{
	if (_manualDisconnect.load(std::memory_order_acquire) || !_client)
	{
		return false;
	}
...
{
	if (!_client || _manualDisconnect.load(std::memory_order_acquire) || _client->IsStopping())
	{
		return;
	}
	_client->SetSid(0);
