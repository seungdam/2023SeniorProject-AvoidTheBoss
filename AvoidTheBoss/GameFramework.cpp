...
void CGameFramework::CreateDirect3DDevice()
{
	...
	hResult = D3D12GetDebugInterface(__uuidof(ID3D12Debug), (void**)&pd3dDebugController); //Before Direct3D Device Creation
	if (pd3dDebugController)
	{
		pd3dDebugController->EnableDebugLayer(); //Windows Settings/System/Apps&Feature: Install 'Graphics Tools'
		...
		&pdxgiAdapter1); i++)
	{
		...
		pdxgiAdapter1->GetDesc1(&dxgiAdapterDesc);
		if (dxgiAdapterDesc.Flags & DXGI_ADAPTER_FLAG_SOFTWARE) continue;
		if (SUCCEEDED(D3D12CreateDevice(pdxgiAdapter1, D3D_FEATURE_LEVEL_12_0,
	...
	if (!pdxgiAdapter1)
	{
		m_pdxgiFactory->EnumWarpAdapter(_uuidof(IDXGIAdapter1), (void**)&pdxgiAdapter1);
		D3D12CreateDevice(pdxgiAdapter1, D3D_FEATURE_LEVEL_12_0, _uuidof(ID3D12Device), (void
			**)&m_pd3dDevice);
	}
	//특성 레벨 12.0을 지원하는 하드웨어 디바이스를 생성할 수 없으면 WARP 디바이스를 생성한다.

	IDXGIAdapter3* pdxgiAdapter3 = NULL;
	hResult = pdxgiAdapter1->QueryInterface(&pdxgiAdapter3);
	if (pdxgiAdapter3)
	{
		DXGI_QUERY_VIDEO_MEMORY_INFO dxgiLocalVideoMemoryInfo, dxgiNonLocalVideoMemoryInfo;
		hResult = pdxgiAdapter3->QueryVideoMemoryInfo(0, DXGI_MEMORY_SEGMENT_GROUP_LOCAL, &dxgiLocalVideoMemoryInfo);
		hResult = pdxgiAdapter3->QueryVideoMemoryInfo(0, DXGI_MEMORY_SEGMENT_GROUP_NON_LOCAL, &dxgiNonLocalVideoMemoryInfo);
		pdxgiAdapter3->Release();
	}
	if (pdxgiAdapter1) pdxgiAdapter1->Release();
...
void CGameFramework::ProcessInput()
{
	...
	if ((dwDirection != 0) || (cxDelta != 0.0f) || (cyDelta != 0.0f))
	{
		...
		if (dwDirection) m_pPlayer->Move(dwDirection, (60.0f * UNIT) * m_Timer.GetTimeElapsed(),
			true);
	}
...
void CGameFramework::OnProcessingKeyboardMessage(HWND hWnd, UINT nMessageID, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_KEYUP:
		...
		case VK_F1:
			if (m_pPlayer) m_pCamera = m_pPlayer->ChangeCamera(FIRST_PERSON_CAMERA, m_Timer.GetTimeElapsed());
		case VK_F3:
			if (m_pPlayer) m_pCamera = m_pPlayer->ChangeCamera(THIRD_PERSON_CAMERA, m_Timer.GetTimeElapsed());
			break;
		case VK_F9:
