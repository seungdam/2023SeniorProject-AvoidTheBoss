...
CTexture::CTexture(int nTextures, UINT nTextureType, int nSamplers)
{
	...
	m_nTextures = (nTextures > 0) ? nTextures : 0;

	const auto textureCount = static_cast<std::size_t>(m_nTextures);
	m_textures.resize(textureCount);
	m_textureUploadBuffers.resize(textureCount);
	m_rootArgumentInfos.resize(textureCount);
	m_samplerGpuDescriptorHandles.resize(static_cast<std::size_t>((nSamplers > 0) ? nSamplers : 0));
}

void CTexture::SetRootArgument(int nIndex, UINT nRootParameterIndex, D3D12_GPU_DESCRIPTOR_HANDLE d3dSrvGpuDescriptorHandle)
{
	auto& rootArgument = m_rootArgumentInfos.at(static_cast<std::size_t>(nIndex));
	rootArgument.m_nRootParameterIndex = nRootParameterIndex;
	rootArgument.m_d3dSrvGpuDescriptorHandle = d3dSrvGpuDescriptorHandle;
}

void CTexture::SetSampler(int nIndex, D3D12_GPU_DESCRIPTOR_HANDLE d3dSamplerGpuDescriptorHandle)
{
	m_samplerGpuDescriptorHandles.at(static_cast<std::size_t>(nIndex)) = d3dSamplerGpuDescriptorHandle;
}
...
void CTexture::UpdateShaderVariables(ID3D12GraphicsCommandList4   *pd3dCommandList)
{
	...
	else
	{
		...
		{
			const auto& rootArgument = m_rootArgumentInfos[static_cast<std::size_t>(i)];
			pd3dCommandList->SetGraphicsRootDescriptorTable(rootArgument.m_nRootParameterIndex, rootArgument.m_d3dSrvGpuDescriptorHandle);
		}
	}
}
...
{
	const auto& rootArgument = m_rootArgumentInfos.at(static_cast<std::size_t>(nIndex));
	pd3dCommandList->SetGraphicsRootDescriptorTable(rootArgument.m_nRootParameterIndex, rootArgument.m_d3dSrvGpuDescriptorHandle);
}
...
void CTexture::LoadTextureFromFile(ID3D12Device5 *pd3dDevice, ID3D12GraphicsCommandList4   *pd3dCommandList, const wchar_t *pszFileName, UINT nIndex, bool bIsDDSFile)
{
	if (bIsDDSFile)
	{
		...
		texture.Attach(::CreateTextureResourceFromDDSFile(pd3dDevice, pd3dCommandList, assetPath.c_str(), uploadBuffer.GetAddressOf(), D3D12_RESOURCE_STATE_PIXEL_SHADER_RESOURCE));

		const auto index = static_cast<std::size_t>(nIndex);
		m_textures.at(index) = std::move(texture);
		m_textureUploadBuffers.at(index) = std::move(uploadBuffer);
	}
}
...
{
	CSkyBoxMesh *pSkyBoxMesh = new CSkyBoxMesh(pd3dDevice, pd3dCommandList, 20.0f, 30.0f, 2.0f);
	SetMesh(pSkyBoxMesh);
